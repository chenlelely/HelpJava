...
外观模式（Facade Pattern)：外部与一个子系统的通信通过一个统一的外观角色进行，为子系统中的一组接口提供一个一致的入口，外观模式定义了一个高层接口，这个接口使得这一子系统更加容易使用。外观模式又称为门面模式，它是一种对象结构型模式。

![image-20200411131530825](09设计模式之Facade外观模式.assets/image-20200411131530825.png)
